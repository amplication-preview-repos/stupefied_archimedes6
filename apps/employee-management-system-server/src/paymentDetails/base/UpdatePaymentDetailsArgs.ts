/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PaymentDetailsWhereUniqueInput } from "./PaymentDetailsWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { PaymentDetailsUpdateInput } from "./PaymentDetailsUpdateInput";

@ArgsType()
class UpdatePaymentDetailsArgs {
  @ApiProperty({
    required: true,
    type: () => PaymentDetailsWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => PaymentDetailsWhereUniqueInput)
  @Field(() => PaymentDetailsWhereUniqueInput, { nullable: false })
  where!: PaymentDetailsWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => PaymentDetailsUpdateInput,
  })
  @ValidateNested()
  @Type(() => PaymentDetailsUpdateInput)
  @Field(() => PaymentDetailsUpdateInput, { nullable: false })
  data!: PaymentDetailsUpdateInput;
}

export { UpdatePaymentDetailsArgs as UpdatePaymentDetailsArgs };
