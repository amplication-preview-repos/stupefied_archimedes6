/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UserProjectsWhereInput } from "./UserProjectsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { UserProjectsOrderByInput } from "./UserProjectsOrderByInput";

@ArgsType()
class UserProjectsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => UserProjectsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => UserProjectsWhereInput, { nullable: true })
  @Type(() => UserProjectsWhereInput)
  where?: UserProjectsWhereInput;

  @ApiProperty({
    required: false,
    type: [UserProjectsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [UserProjectsOrderByInput], { nullable: true })
  @Type(() => UserProjectsOrderByInput)
  orderBy?: Array<UserProjectsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { UserProjectsFindManyArgs as UserProjectsFindManyArgs };
